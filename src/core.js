...
function buildAssistantPrompt({
  ...
  const sys = [
    ...
    `- Objectif : perte de gras. Repère quotidien ~${r0(targetKcal)} kcal / ~${r0(targetP)} g de protéines — un cadre, pas une grille rigide. Privilégie des repas protéinés.`,
    ...
    // Sermons soft (rétention d'eau + variété) : gardés pour la planif jour/semaine (pédagogie budget
    // assumée), RETIRÉS du mode "meal" — ils sur-contraignaient l'invention d'une idée de repas.
    ...(mode === "meal" ? [] : [
    ...
    // Mode "meal" : posture "chef créatif" — invente librement, ne refuse jamais.
    ...(mode === "meal" ? [
      "TON RÔLE (mode idée de repas) : tu es un CHEF créatif. INVENTE des repas savoureux et variés avec des ingrédients de supermarché standard — comme si tu répondais librement, sans catalogue. Explore les cuisines du monde, ne te limite PAS aux habitudes de Bob, et à chaque nouvelle demande propose autre chose.",
      "TU NE REFUSES JAMAIS : tu proposes TOUJOURS au moins le nombre d'options demandé, concrètes et prêtes à cuisiner. Ne réponds JAMAIS qu'aucune recette ne convient. Si les contraintes sont serrées ou se contredisent, relâche la plus SOUPLE (le budget d'abord, jamais une règle diététique ni une exclusion explicite), propose quand même, et explique le compromis en une phrase dans `note`.",
    ] : []),
    "Consignes de calcul :",
  ...
  if (have.length) {
    ...
    if (mode === "meal" || mode === "day" || mode === "week") L.push(fridgeOnly
      ...
      : "FRIGO = BONUS, pas une contrainte — le frigo ci-dessus est une INSPIRATION. Si un ou plusieurs de ces aliments collent bien à la demande, privilégie-les et signale-le (« tu as déjà X »). Sinon, invente LIBREMENT avec des ingrédients de supermarché standard : n'exige PAS que la protéine ou les légumineuses viennent du frigo, ne te prive PAS d'un bon plat parce qu'un ingrédient n'y est pas.");
    L.push("");
  }
  ...
  if (mode === "week") {
    ...
    if (userWish && userWish.trim()) L.push(`MA DEMANDE (à respecter en PRIORITÉ absolue) : « ${userWish.trim()} ». Respecte-la à la lettre même si ça sort de mes habitudes. Décode-la finement : une EXCLUSION (« sans X », « pas de X », « pas envie de X », « je n'aime pas X ») = n'utilise PAS X ni un proche ; une limite CHIFFRÉE (« <650 kcal », « moins de 30 g de gras ») PRIME sur le repère de budget ci-dessus ; « le plus de protéines possible » / « max protéines » = maximise vraiment les protéines. Seules les règles diététiques dures peuvent la contredire.`);
    ...
    else if (reserveKcal > 50) L.push(`À GARDER EN TÊTE — le repère ci-dessus est ma MARGE pour CE repas : j'ai d'autres repas pas encore décidés aujourd'hui (≈${r0(reserveKcal)} kcal leur sont réservés). Essaie de rester dans cette marge. Si elle est quasi nulle, propose quand même la meilleure option légère et protéinée possible et signale-le dans la note (ne refuse jamais).`);
  ...
  if (!dining && !fridgeOnly && have.length) L.push(mode === "meal"
    ? "Si des aliments de mon frigo/placard collent bien à la demande, sers-t'en et signale-le (« tu as déjà X ») — mais reste LIBRE d'inventer avec des ingrédients standard : le frigo est un bonus, pas une obligation. ANTI-GASPI : priorise les produits FRAIS et périssables présents (fruits, légumes, herbes fraîches, laitages frais, tofu ouvert…) — ce sont eux qui s'abîment vite, autant les utiliser en premier avant qu'ils ne se perdent."
    : "UTILISE EN PRIORITÉ les aliments listés dans mon frigo/placard ci-dessus : base au moins une PARTIE de chaque repas dessus quand c'est pertinent (indique la portion en g/ml). Tu PEUX compléter avec d'autres aliments courants pour varier et atteindre les cibles, mais ne fais pas comme si mon frigo n'existait pas.");

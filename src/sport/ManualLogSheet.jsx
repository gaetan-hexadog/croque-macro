...
export function ManualLogSheet({ open, onClose, currentWeek, workouts, program, onSave, showToast, sportTheme, exerciseCharges = {} }) {
  ...
  const session = SESS[sid];

  const submitSession = () => {
    const id = `manual-${sid}-${Date.now()}`;
    const base = { id, date: new Date().toISOString(), completed: true, sessionId: sid, week, programId: pid, manual: true };
    let entry;
            ...
            {ORDER.map((k) => {
              ...
              return (
                <button key={k} onClick={() => setSid(k)} className="flex w-full items-center gap-3 rounded-2xl p-3 text-left active:scale-[0.99]" style={{ backgroundColor: T.paper, border: `1px solid ${on ? T.accent : T.line}` }}>
                  ...
                  <span className="flex-1"><span className="block text-sm font-bold" style={{ color: T.ink }}>{SESS[k].name}</span><span className="block text-xs" style={{ color: T.muted }}>{SESS[k].subtitle}</span></span>
